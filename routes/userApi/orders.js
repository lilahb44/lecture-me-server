...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const orders = await asyncQuery(
      `select o.id, g.name AS groupName, CONCAT(l.firstName," ",l.lastName) AS lecturer, o.address, DATE_FORMAT(o.date, '%d-%m-%Y %h:%i:%s') as date, IF(o.status=1,"Accept", IF(o.status=0,"Decline", IF(o.status=2,"Paid", "Pending"))) as status, o.price
      from orders o
